(function() {
  ...
  var Templates = {
    ...
    statusScreen: function(data) {
      var title = this.title('A prémium előfizetés állapota');
      var content = '';
      if (data.subscriber) {
        content += this.primaryMessage('Előfizetve', null);
        content += this.message('Lejárati dátum:<br/>' + data.expiryDate);
        content += this.button('Előfizetés meghosszabbítása', 'renew', 'font-weight:bold;');
      }
      else if (data.expired) {
        content += this.primaryMessage('Lejárt', null);
        content += this.message('Lejárati dátum:<br/>' + data.expiryDate);
        content += this.button('Előfizetés meghosszabbítása', 'renew', 'font-weight:bold;');
      }
      else {
        content += this.primaryMessage('Nincs előfizetése', null);
        content += this.button('Előfizetés', 'renew', 'font-weight:bold;');
      }
      return this.dialogBox(title + content + this.closeButton());
    },

    productsScreen: function(products) {
      var title = this.title('Választható előfizetések');
      var content = '';
  ...
  Controller.prototype.deliverOrder = function(p) {
    ...
    this.expiryStore.load(function(err, expiryDate) {
      if (err) {
        return this.view.showError("Hiba történt az előfizetés során: " + err);
      }
      var now = +new Date();
  ...
  nonRenewing.initialize = function(options) {
    ...
    if (!window.store) {
      this.view.showError('Hiba az előfizetési modulban');
      return;
    }
